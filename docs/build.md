...
The backend lives at `ggml/src/ggml-gcu/`, organized in the same shape as CANN: `common.{h,cpp}` for shared types and helpers, `gcu_pool.{h,cpp}` for the device memory pool, `gcu_buffer.{h,cpp}` for the ggml buffer interface, `gcu_ops.h` for op-handler declarations, and one `gcu_op_<family>.cpp` per op family (`elementwise`, `norm`, `matmul`, `attn`, `misc`). `ggml-gcu.cpp` ties them together and hosts the backend / device interface.
...
./build/bin/test-backend-gcu                 # smoke suite (90+ sub-tests covering every dispatched op + F32/F16/BF16 dtype variants + Q-weight roundtrip + async H<->D overlap + real-model shape coverage)
GCU_BENCH=1 ./build/bin/test-backend-gcu     # opt-in: also print per-op micro-bench at decode shapes
...
- Position encoding: `ROPE` (NORMAL mode 0 + NEOX mode 2; `freq_factors` (proportional rope) and partial rotation (`n_dims < head_dim`) supported as of MVP-5b; MROPE/VISION/IMROPE scaffolded but gated behind `GGML_GCU_ENABLE_MROPE` env var pending SDK position-layout diagnostic; no YARN (`ext_factor != 0`); F32, F16, and BF16)
...
- Indexing: `GET_ROWS` (F32 only, unbatched), `SET_ROWS` ({F32, F16, BF16} dst with same-class src; on-device KV cache write via `topsatenIndexPut` with I64→I32 index cast)
- Memory: `CPY`/`DUP`/`CONT` (same-dtype contiguous + any pairwise F32/F16/BF16 conversion); view ops (`RESHAPE`, `VIEW`, `PERMUTE`, `TRANSPOSE`)
- Activation dtypes: F32, F16, **BF16** (BF16 added in MVP-5a; covers ADD, SUB, MUL, DIV, SCALE, RMS_NORM, NORM, SOFT_MAX, all unary activations, GLU, ROPE, MUL_MAT and MUL_MAT_ID weight + activation, CPY conversions). End-to-end verified on Llama 3.2 1B BF16 (`unsloth/Llama-3.2-1B-Instruct-GGUF`): `llama-completion --temp 0` produces matching first 32 tokens on `--device GCU0 -nkvo` and `--device CPU`. Bench (`--device GCU0 -nkvo 1`, r=5): pp64 1389.82 ± 58.96 t/s, tg16 59.77 ± 0.15 t/s.
...
- KV cache can now live on GCU. `SET_ROWS` to F16/BF16 destinations is accepted (MVP-6). The handler calls `topsatenIndexPut` after a queued I64→I32 cast on the compute stream (an SDK constraint we re-probed in 2026-05: `topsatenIndexPut` accepts I32 indices but rejects I64 with "indice data type not support: 11"). A queued F32→F16 cast handles ggml's typical "F32 src into F16 cache" pattern. Both casts and the scatter are fully on-stream, so the per-call sync H2D that made the MVP-3b probe 2-5× slower than `-nkvo` is gone. `-nkvo` remains valid as a CPU-only-KV-cache fallback if needed; Ollama and other consumers that auto-assign the cache to the device now work out of the box.
- BF16 supported on core ops as of MVP-5a (see operator coverage above). End-to-end verified on Llama 3.2 1B BF16 (`unsloth/Llama-3.2-1B-Instruct-GGUF`): `llama-completion --temp 0` produces bit-identical output to CPU. Bench numbers in the coverage line.
- `ROPE` supports NORMAL + NEOX + `freq_factors` + partial rotation (see operator coverage). YARN (`ext_factor != 0`) goes to CPU. MROPE/VISION/IMROPE handlers exist but are gated behind `GGML_GCU_ENABLE_MROPE`; an SDK position-layout discrepancy with `topsvllmMRotaryEmbedding` is documented as a TODO at the gate and remains an open diagnostic.
...
- Single device, single stream. Pinned host memory (`topsHostMalloc`) is enabled and used by the `-nkvo` KV cache. On Llama 3.2 1B Q4_K_M (`--device GCU0 -nkvo 1`, r=5):
  ...
  **MVP-4b — queued ops (drop per-op compute sync).** Replaces every op handler's `topsStreamSynchronize` + `pool.free` with a deferred-free routed through `gcu_release_scratch`. Kernels now queue on `compute_stream` without per-op host round-trips; scratch returns to the pool at `graph_compute` end after a single drain synchronize. Llama 3.2 1B Q4_K_M (`--device GCU0 -nkvo 1`, r=5):
  ...
  After Tranche B+D+F+H op-coverage work (FLASH_ATTN_EXT, GLU, the MoE weight-norm path moved to GCU, etc.) plus MVP-5a (BF16) and MVP-5b (ROPE freq_factors / partial), HEAD `a17700801`, r=5:
  ...
  tg16 is **+82.9 %** vs the pre-MVP-5 baseline (7.65 → 13.99 t/s). Driving factors: `FLASH_ATTN_EXT` collapses Q@K + scaled softmax + @V into one fused kernel per attention layer; `SUM_ROWS` / `MEAN` / `DIV` / `CLAMP` for MoE weight normalization moved from CPU fallback to GCU, eliminating cross-backend round-trips per layer; `GLU` keeps the FFN gate*up step on-device; ROPE `freq_factors` now runs on GCU for Gemma 4's full-attention layers instead of falling back to CPU.
