// ggml-gcu: device + host buffer interfaces — implementation.
//
// Houses the buffer-type and buffer slots wired into ggml-backend's buffer
// interface, plus the host (pinned) buffer type used for pinned KV memory
// when running with -nkvo. Carries the MVP-3a Q-typed host cache that lets
// cross-backend tensor copies preserve the original Q-quant bytes.
...
// === Buffer type ====================================================
// ggml_backend_gcu_buffer_type_context lives in gcu_buffer.h so the device
// context can hold one by-value without dragging this TU's implementation
// details in.
